...
EXPS = ["../exp1/", "../exp1_5/", "../exp1_10/", "../exp1_15/"]
...
def main():
    ...
    # Results
    exp_baseline = configs[0]
    befores = []
    for i in range(1, len(configs)):
        for j in range(len(exp_baseline)):
            env = make_env(configs[i][j])
            config = exp_baseline[j]
            state_dim = np.prod(env.observation_space.shape)
            action_dim = env.action_space.n
            agent = load_agent(config, env, EXPS[0], state_dim, action_dim, device)
            avg_reward = compute_average_reward(agent, env, num_runs=config['window_length'], nb_pos_r=False)
            befores.append(avg_reward)
    np.save("results/eval3_before.npy", np.array(befores))
    ...
    np.save("results/eval3.npy", np.array(results))
